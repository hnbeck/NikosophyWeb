...
| III | In the Metaverse, elements can be placed, interacting immediatly together and with the environment (the Metaverse itself) according the Metaverse Mechanics. The location of any place shall be reliable and referenceable | Suddenly when an object comes to "life", its presence triggers also the interaction with all other simulating objects and enviroment. It is important that every object has a location, which can be referenced and keeps always the same as long as the object not moves. The reference (like coordinates or other descriptions) is thy way any object can be found. |
